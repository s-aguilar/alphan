...
def FilterBackground(image):
    ...
    if hull is None:
        print("In FilterBackground(), hull is returning 'None'") 
        return None

    ys,xs = np.where(image)
    path1 = mplPath.Path(hull[:,0,:])
    mask = path1.contains_points(list(zip(xs,ys))) # casted zip(xs,ys) as a list py2 vs py3 issue
...
def GetEventPositions(pic,debug_mode=0):
    ...
    if points is None:
        print("Messed up in GetEventPositions(), 'points' is off") 
        return None

    xc,yc = VertexPos(pic_,points)
    return points, (xc.item(),yc.item()) ############ cast them to python types
...
def convexHull(thresh, debug_mode = 0):
    ...
    try:
        ...
        print("convexHull() got a exception for ValueError")
        return None

    gpc = [np.concatenate(g,axis=0) for g in gpc]

    if debug_mode: 
        plt.scatter(gpc[0][:,0,0],gpc[0][:,0,1])
        plt.show()
...
def TipFinder(thresh, debug_mode = 0):
    ...
    if hull is None: 
        print("I messed up in TipFinder(), hull is not right")
        return None
    #############################
    bA,bB,bC = MaxEnclosedTriangle(hull)

    if debug_mode: 
        plt.scatter(hull[[bA,bB,bC],0,0],hull[[bA,bB,bC],0,1],marker='*', s=500,c='g')
        plt.show()
