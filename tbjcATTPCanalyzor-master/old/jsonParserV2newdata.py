...
with open('data(2).dat') as f:
    ...
    for d in l:
        ...
        #        'theta1':x[0][0],'L1':x[0][1],
        #        'theta2':x[1][0],'L2':x[1][1],
        #        'theta3':x[2][0],'L3':x[2][1],
        #        'theta4':x[3][0],'L4':x[3][1]})
'''
df = pd.DataFrame(info)

mask = ((df['L3']>2) & df['L1'].between(4,10000) & df['L2'].between(4,10000)
...
#     rad.append([row["theta1"],row["theta2"],row["theta3"],row["theta4"]])
#     r.append([row["L1"],row["L2"],row["L3"],row["L4"]])
#
# for index in range(len(rad)):
#     ax = plt.subplot(111, projection='polar')
#     radians = np.radians(rad)
#     ax.scatter(radians[index],r[index],marker = '.',c = 'r')
#     ax.set_rmax(max(r[index])+1)
#     plt.show()
